...
6. **描画（Render）** — `src/components/CharacterActor.tsx` が各キャラをスプライトシートから
   ...
   モーション個性（歩く速さ・行動半径・休憩のリズム・好きなジェスチャ）に従って
   自席のまわりで過ごします。全員が自席から決まった範囲内に留まるので、画面は
   ごちゃつきません。カスタムキャラには id から決定論的に個性が導出されるので、
...
全員が自席から決まった半径内に留まり、その中での振る舞いに個性が出ます。Haya は
せわしなく動いてジャンプし、Suzu はほとんど動かずじっくりレビューし、Kii はよく同僚に
手を振り、Boss は自分のまわりをきびきび回る、といった具合です。カスタムキャラは
