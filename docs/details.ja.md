...
6. **描画（Render）** — `src/components/CharacterActor.tsx` が各キャラをスプライトシートから
   ...
   モーション個性（移動パターン＝端から端への往復・せわしない小刻み移動・気ままな
   うろつき、歩く速さ、休憩のリズム、シグネチャの仕草＝手振り / レビュー / ジャンプ /
   待機）に従って自席のまわりで過ごします。全員が自席から決まった範囲内に留まるので、
   画面はごちゃつきません。カスタムキャラには id から決定論的に個性が導出されるので、
   アップロードしたキャラも常に同じ動き方をします。
...
全員が自席から決まった半径内に留まり、その中での振る舞いに個性が出ます。Boss と Sora は
自分のエリアを端から端まで往復し、Haya と Rin はせわしなく小刻みに動いてよくジャンプし、
Suzu はほとんど置物のようにじっくりレビューし、Kii はしょっちゅう同僚に手を振り、Mio は
のんびり待つ、といった具合です。各キャラは自分のシグネチャの仕草を高頻度で使うので、
ひと目で見分けがつきます。カスタムキャラは id のハッシュから個性（移動パターンと
シグネチャの仕草を含む）が導出されるので、アップロードしたキャラも一貫した独自の
動き方をします。
