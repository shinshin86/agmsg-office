...
1. **読み込み（Load）** — `agmsg` のメッセージテーブル形式の生レコードを、ログ源から
   読み込みます（[ログ形式](#ログ形式)参照）。ログ源は、同梱サンプル / ローカル `agmsg` の
   dev API / 手動でインポートした JSON のいずれかです。
...
ここまでの処理はすべてブラウザ側（React の状態）で完結します。サーバ側のコードは任意の
dev API（`vite.config.ts`）だけで、`npm run dev` の時のみ動きます。
...
各スプライトシートは、1 枚の透過全身立ち絵（`portrait.png`）から生成されます。再生成は以下で
行います。
